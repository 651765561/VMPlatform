...
{
    public partial class JW_GoodsMain_ZDJSBll : RepositoryFactory<JW_GoodsMain>
    {
        /// <summary>
        /// 加载主表信息
        /// </summary>
        ...
        /// <param name="apply_id"></param>
        /// <returns></returns>
        public string CheckState2(string apply_id)
        {
            string sql = string.Format(@"select * from  JW_Apply_detail jad 
                                         join JW_Apply_room jar  on jad.Apply_room_id=jar.Apply_room_id
                                         join jw_apply ja on ja.apply_id=jar.apply_id 
                                         where jad.intime is null and jad.outtime is not null 
                                         and ja.apply_id='{0}'", apply_id);
            try
            {
                DataTable dt = SqlHelper.DataTable(sql, CommandType.Text);
                if (dt.Rows.Count <= 0)
