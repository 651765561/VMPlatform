...
namespace LeaRun.Business
{
    ...
    public class Base_ModulePermissionBll : RepositoryFactory<Base_ModulePermission>
    {
        ...
        /// <param name="ObjectId">对象主键</param>
        /// <returns></returns>
        public List<Base_Module> GetModuleList(string ObjectId,string regcode)
        {
            StringBuilder strSql = new StringBuilder();
            if (!ManageProvider.Provider.Current().IsSystem)
            {
                strSql.Append(@"SELECT DISTINCT  M.* FROM Base_Module M");
                strSql.Append(@" INNER JOIN Base_ModulePermission MP ON M.ModuleId = MP.ModuleId 
                                 WHERE  (M.ModuleId not in (select ModuleId from Base_Module where ModuleId not in 
                                  (select * from dbo.f_split('" + regcode + @"',',',':')) and Level=1 and parentid<>'0'))                     
                                and  ObjectId IN ('" + ManageProvider.Provider.Current().ObjectId.Replace(",", "','") + "')");
            }
            else
            {
