...
namespace LeaRun.Business
{
    ...
    /// </author>
    /// </summary>
    public class QUERY_JW_ApplyBll : RepositoryFactory<QUERY_JW_Apply>
    {
        /// <summary>
        /// 列表加载
        /// </summary>
            ...
            try
            {
                ...
                string sqlTotal =
                    string.Format(
                        @" 
                                        select 
                                            ROW_NUMBER() over(order by adddate desc) rowNumber, *  from JW_Apply
                                            where type=1 and state in  (3,4,5)
                                            and  PoliceArea_id in (select PoliceArea_id from Base_PoliceArea where AreaType in (1,5))
