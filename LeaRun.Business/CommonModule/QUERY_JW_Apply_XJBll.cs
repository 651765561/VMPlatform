...
namespace LeaRun.Business
{
    ...
    /// </author>
    /// </summary>
    public class QUERY_JW_Apply_XJBll : RepositoryFactory<QUERY_JW_Apply_XJ>
    {
        /// <summary>
        /// 列表加载
        /// </summary>
                    ...
                    string.Format(
                        @" select * from ( 
                                        select 
                                            ...
                                            , * 
                                        from JW_Apply 
                                            where unit_id='{0}' and type=2 and state in (3,4,5) 
                                         and  PoliceArea_id in (select PoliceArea_id from Base_PoliceArea where AreaType in (2))
               ...
               string sqlTotal =
                    string.Format(
                        @"   select   ROW_NUMBER() over(order by adddate desc) rowNumber, *  from JW_Apply 
                          where type=2 and state in  (3,4,5) 
                          and  PoliceArea_id in (select PoliceArea_id from Base_PoliceArea where AreaType in (2))
                                       "
                        );
        ...
        public DataTable GetListArea(string unit_id)
        {
            ...
            strSql.Append(@"SELECT  *
                                FROM    ( 
                                        ...
                                        ) T WHERE 1=1 and T.AreaType in (2)");
            List<DbParameter> parameter = new List<DbParameter>();
            //if (!string.IsNullOrEmpty(unit_id))
            //{
