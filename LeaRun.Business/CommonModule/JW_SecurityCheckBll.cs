...
{
    public class JW_SecurityCheckBll : RepositoryFactory<JW_SecurityCheck>
    {
        /// <summary>
        /// 加载列表数据
        /// </summary>
        ...
        /// <returns></returns>
        public string GridPageSecurityCheckJson(string ParameterJson, string apply_id, JqGridParam jqgridparam)
        {
            try
            {
...
order by {2} {3} "
        ...
        /// <returns></returns>
        public int SubmitSecurityCheckForm(string submitType, JW_SecurityCheck jwSecurityCheck)
        {
            //先获取相关信息
...
");
        ...
        /// <returns></returns>
        public int DeleteSecurityCheck(string securitycheck_id, string user_id)
        {
            // start 取消不同法警互相删除的权限
