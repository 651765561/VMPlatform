...
{
    public class JW_GoodsMainBll : RepositoryFactory<JW_GoodsMain>
    {
        /// <summary>
        /// 加载主表信息
        /// </summary>
        ...
        /// <param name="apply_id"></param>
        /// <returns></returns>
        public DataTable LoadQM(string apply_id)
        {
            string sql = string.Format(@"select * from JW_Qm where Object_id='{0}' and type=1", apply_id);
            try
            {
                DataTable dt = SqlHelper.DataTable(sql, CommandType.Text);
                return dt;
