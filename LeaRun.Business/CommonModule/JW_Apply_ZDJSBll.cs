...
{
    public partial class JW_Apply_ZDJSBll : RepositoryFactory<JW_Apply>
    {
        /// <summary>
        /// 根据当前单位的主键获取单位的相关信息
        /// </summary>
...
join Case_CasePower ccp on cci.case_id=ccp.case_id 
where ccp.powerlevel='1' and powertype='1' and ccp.user_id='{0}' ", user_id);
            try
            {
                DataTable dt = SqlHelper.DataTable(sql, CommandType.Text);
                return dt;
...
apply_id,type,unit_id,unitname,dep_id,depname,asker_id,PoliceArea_id,adduser_id,adddate,startdate,enddate,docCode,roomdetail,userName,userSex,userAge,userCode,userNation,userbiref,userPoliticalstatus,userIsNPCmember,userEducation,userWork,userDuty,userHome,userHealthy,remark,detail,ApprovalDetail,state,tasktype_id
)values(
@apply_id,@type,@unit_id,@unitname,@dep_id,@depname,@asker_id,@PoliceArea_id,@adduser_id,@adddate,@startdate,@enddate,@docCode,@roomdetail,@userName,@userSex,@userAge,@userCode,@userNation,@userbiref,@userPoliticalstatus,@userIsNPCmember,@userEducation,@userWork,@userDuty,@userHome,@userHealthy,@remark,@detail,@ApprovalDetail,@state,@tasktype_id)");
        ...
        public int EdutApply(JW_Apply jwApply, string submitType)
        {
            ...
            string sql = string.Format(@"update JW_Apply set type=@type,userName=@userName,userSex=@userSex,userAge=@userAge,userCode=@userCode,userNation=@userNation,userbiref=@userbiref,userPoliticalstatus=@userPoliticalstatus,userIsNPCmember=@userIsNPCmember,userEducation=@userEducation,userWork=@userWork,userDuty=@userDuty,userHome=@userHome,userHealthy=@userHealthy,remark=@remark  where apply_id=@apply_id");
            ...
            try
            {
                ...
                string sqlTotal = string.Format(@"
                         select 
                                            ...
                                            , * 
                                        from JW_Apply 
                                            where unit_id='{0}' and type=3 and state <> 5  and adduser_id='{1}'
        ...
        {
            string sql = string.Format(@" select jwa.PoliceArea_id,pa.AreaName,bu.unit from JW_Apply jwa
                                join Base_PoliceArea pa on jwa.PoliceArea_id=pa.PoliceArea_id 
                                join Base_Unit bu on pa.unit_id=bu.Base_Unit_id 



                                where jwa.apply_id='{0}' ", keyValue);
            try
            {
                DataTable dt = SqlHelper.DataTable(sql, CommandType.Text);
                if (dt != null && dt.Rows.Count > 0)
                {
                    return dt.Rows[0]["PoliceArea_id"].ToString() + "|" + dt.Rows[0]["unit"] + "--" + dt.Rows[0]["AreaName"].ToString();
                }
                else
                {
                    return string.Empty;
...
update JW_SendPolice set state=3 where type=1 and Object_id=@apply_id and user_id=@user_id
        ...
        public DataTable LoadPoliceArea(string unit_id, string policeAreaType)
        {
            ...
            string sql = string.Format(@"  select top 1 bpa.*,unit from Base_PoliceArea bpa  join Base_Unit bu on bpa.unit_id=bu.Base_Unit_id  where bpa.unit_id='{0}' {1} ", unit_id, where);
            try
            {
                DataTable dt = SqlHelper.DataTable(sql, CommandType.Text);
                return dt;
...
join Base_User bu on ja.leaderuser_id=bu.UserId where ja.apply_id='{0}'", apply_id);
        ...
        /// <summary>
        /// 指定居所，新增的时候，默认加载当前登录用户的所在部门信息
        /// </summary>
        /// <param name="user_id"></param>
        /// <returns></returns>
        public string LoadDepMsg(string user_id)
        {
            string sql = string.Format(@"select bd.* from Base_User bu 
join Base_Department bd on bu.dep_id=bd.Dep_id 
where bu.UserId='{0}'", user_id);
            try
            {
                DataTable dt = SqlHelper.DataTable(sql, CommandType.Text);
                if (dt.Rows.Count <= 0)
                ...
                {
                    return dt.Rows[0]["Dep_id"] + "|" + dt.Rows[0]["Name"];
                }
            }
            catch (Exception)
            {
        ...
        /// <summary>
        /// 根据办案区的主键获取其所在单位的名称和办案区的名称
        /// </summary>
        /// <param name="policeAreaId"></param>
        /// <returns></returns>
        public string GetUnitArea(string policeAreaId)
        {
            string sql = string.Format(@" select bpa.AreaName,bu.unit from Base_PoliceArea bpa 
join Base_Unit bu on bpa.unit_id=bu.Base_Unit_id 
where bpa.PoliceArea_id='{0}' ", policeAreaId);
            try
            {
                DataTable dt = SqlHelper.DataTable(sql, CommandType.Text);
                return dt.Rows[0]["unit"] + "--" + dt.Rows[0]["AreaName"];
            }
            catch
            {
                return string.Empty;
