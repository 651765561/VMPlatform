...
{
    public partial class JW_Apply_detail_ZDJSBll : RepositoryFactory<JW_Apply_detail>
    {
        /// <summary>
        /// 加载当前房间的信息
        /// </summary>
        ...
        public string GridPageApplyDetailJson(string ParameterJson, string apply_id, JqGridParam jqgridparam)
        {
            try
            {
                ...
                {
                    sqlWhere = string.Format(" where jar.apply_id='{0}' and jad.record_type in (2,4,7) ", apply_id);
                }
                string sqlTotal = string.Format(@"
        select ROW_NUMBER() over(order by outtime desc) rowNumber, jad.*,br.RoomName Apply_room_name,jar.state from JW_Apply_detail jad 
...
insert into JW_Apply_detail(Apply_detail_id,Apply_room_id,outtime,intime,record_type,cmaker,cmakerBack,detail,adduser_id,addate) 
values(
@Apply_detail_id,@Apply_room_id,@outtime,@intime,@record_type,@cmaker,@cmakerBack,@detail,@adduser_id,@addate
)"
                    );
                SqlParameter[] pars = new SqlParameter[]
                {
                    ...
                    new SqlParameter("@cmakerBack",jwApplyDetail.cmakerBack),
                ...
                try
                {
                    ...
                    string sqlUpdateRoom = string.Format(@"update JW_Apply_room set state=2 where apply_id='{0}' and enddate is NULL ", apply_id);
            ...
            else
            {
                ...
                //编辑
                string sqlEdit = string.Format(@"update JW_Apply_detail set outtime=@outtime,intime=@intime,record_type=@record_type,cmaker=@cmaker,cmakerBack=@cmakerBack,detail=@detail,adduser_id=@adduser_id where Apply_detail_id=@Apply_detail_id");
                SqlParameter[] pars = new SqlParameter[]
                {
                    new SqlParameter("@Apply_detail_id",jwApplyDetail.Apply_detail_id),
                    new SqlParameter("@outtime",jwApplyDetail.outtime==null?(object)DBNull.Value:jwApplyDetail.outtime),
