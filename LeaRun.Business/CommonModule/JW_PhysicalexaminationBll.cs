...
{
    public class JW_PhysicalexaminationBll : RepositoryFactory<JW_Physicalexamination>
    {
        /// <summary>
        /// 加载列表数据
        /// </summary>
...
order by {2} {3} "
        ...
        /// <returns></returns>
        public string DelImgs(string upload_id, string exam_id, string user_id)
        {
            //start 取消互相删除的权限
            //string sqlCheckDel = string.Format(@" select * from JW_Upload where upload_id='{0}' and uploaduser_id='{1}' and type=2 and Object_id='{2}' ", upload_id, user_id, exam_id);
        ...
        /// <returns></returns>
        public DataTable LoadQM(string exam_id)
        {
            string sql = string.Format(@"select * from JW_Qm where detail_id='{0}' and type=2", exam_id);
            try
            {
                DataTable dt = SqlHelper.DataTable(sql, CommandType.Text);
                return dt;
