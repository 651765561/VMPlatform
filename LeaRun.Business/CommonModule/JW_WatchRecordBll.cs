...
{
    public class JW_WatchRecordBll : RepositoryFactory<JW_WatchRecord>
    {
        /// <summary>
        /// 加载列表数据
        /// </summary>
...
@watchrecord_id,@unit_id,@PoliceArea_id,@apply_id,@adduser_id,@addDate,@watchuser,@depname,@watchplace,@startdate,@enddate,@dutydetail,@attention,@successor,@depname2)");
                    ...
                    else
                    {
                        string sqlCheckEdit =
                            string.Format(
                                @" select * from JW_WatchRecord where watchrecord_id='{0}' and adduser_id='{1}' "
                                , jwWatchRecord.watchrecord_id
                                , jwWatchRecord.adduser_id
                                );
        ...
        public string GetPrevData(string apply_id)
        {
            ...
            {
                DataTable dt = SqlHelper.DataTable(sql, CommandType.Text);
                string successor = dt.Rows[0]["successor"].ToString().Replace("&nbsp;","");  //赋值给看管人
