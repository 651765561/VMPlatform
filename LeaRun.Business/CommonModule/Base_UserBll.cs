        ...
        public DataTable GetPageList(string keyword, string CompanyId, string DepartmentId, ref JqGridParam jqgridparam)
        {
            ...
            strSql.Append(@"SELECT  *
                            FROM    ( SELECT    u.UserId ,					--用户ID
                                                ...
                                                u.Email ,					--电子邮箱
                                                u.CompanyId ,			    --所在公司ID
                                                c.longname AS CompanyName ,	--所在公司
c.unit,--所在公司名字
                                                u.dep_id,				--所在部门ID
                                                d.Name AS DepartmentName,--所在部门
                                                e.Duty,                     --职务
                                                u.Enabled ,					--是否有效
                                                u.LogOnCount ,				--登录次数
                                                u.LastVisit ,				--最后登录时间
                                                ...
                                                u.Remark					--备注
                                      FROM      Base_User u
                                                LEFT JOIN Base_Unit c ON c.Base_Unit_id = u.CompanyId
                                                LEFT JOIN Base_Department d ON d.dep_id = u.dep_id
            ...
            {
                strSql.Append(" AND dep_id = @DepartmentId");
        ...
        /// <summary>
        /// 获取用户角色列表--用户管理编辑页面下拉框用（单选）
        /// </summary>
        /// <param name="CompanyId">公司ID</param>
        /// <param name="UserId">用户Id</param>
        /// <returns></returns>
        public DataTable UserRoleListjs(string CompanyId, string UserId, int ri)
        {
            StringBuilder strSql = new StringBuilder();
            List<DbParameter> parameter = new List<DbParameter>();
            if (ri == 0)
            {
                strSql.Append(@"SELECT  r.RoleId ,				--角色ID
                                    ...
                                    ou.userid
                            FROM    Base_Roles r
                                    JOIN Base_ObjectUserRelation ou ON ou.ObjectId = r.RoleId
                                                                            AND ou.UserId = @UserId
                                                                            AND ou.Category = 2
                            WHERE 1 = 1");
                strSql.Append(" AND r.CompanyId = @CompanyId");
        ...
        {
            string sql = "";
            sql = string.Format(@"select *  from Base_User
        ...
        public DataTable GetUserList(string account, string before_account)
        {
            try
            {
                string sql = "";
                if (before_account != null && before_account != "")
                {
                    sql = string.Format(@" select *  from Base_User where Account='{0}' and Account != '{1}' ", account, before_account);
                }
                else
                {
                    sql = string.Format(@" select *  from Base_User where Account='{0}'  ", account);
                }
        ...
        public int DeleteUser(string keyValue)
        {
            ...
            string sql = string.Format(@"delete Base_User where UserId='{0}' ; delete Base_ObjectUserRelation  where UserId='{0}'"
                , keyValue
                );
            sb.Append(sql);
