...
{
    public partial class ManagementXJBll
    {
        /// <summary>
        /// 列表加载
        /// </summary>
...
select ROW_NUMBER() over(order by ja.apply_id) rowNumber,ja.*,js.SendDate ,bpa.AreaName from JW_Apply ja 
	                    ...
	                    select ROW_NUMBER() over(order by jpa.apply_id) rowNumber,ja.*,jpa.usedate as SendDate
                        ,bpa.AreaName from JW_Policeapply jpa 
	                    join JW_SendPolice jsp on jpa.apply_id=jsp.Object_id and  (jsp.type=4 or jsp.type=5)
	                    join JW_apply ja on  ja.type=2 and ja.state in (3) and
	                    ((ja.apply_id=jpa.Object_id_12  and jpa.tasktype_id=12) 
	                    or (ja.apply_id=jpa.Object_id_9  and jpa.tasktype_id=9))
                        join Base_PoliceArea bpa on ja.PoliceArea_id=bpa.PoliceArea_id
                ...
                Base_UserBll bllbaseuser = new Base_UserBll();
                foreach (DataRow r in dt.Rows)
                {
                    string[] arr = r["asker_id"].ToString().Split(',');
                    string realNames = "";
                    for (int i = 0; i < arr.Length; i++)
                    {
                        string userid = arr[i];
                        string realname = bllbaseuser.GetRealName(userid);
                        realNames += realname + ",";
                    }
                    r["RealName"] = realNames.Substring(0, realNames.Length - 1);
                }
        ...
        public string GetRoomState(string user_id, string unit_id)
        {
            ...
            string sql = string.Format(@"
                    ...
                    join JW_apply ja on ja.type=2 and ja.state in (4) --4实施中
                    and ((ja.apply_id=jpa.Object_id_12  and jpa.tasktype_id=12) 
	                    or (ja.apply_id=jpa.Object_id_9  and jpa.tasktype_id=9))
...
where ju.apply_id='{0}' and br.state=1 order by ju.addDate", apply_id);
            try
            {
...
where ja.apply_id='{0}' and Room_id not in (select room_id from JW_Usedetail where isend=0) and br.state=1 and br.roomtype_id='2d130e49-2607-4aa2-94a7-0a439010f25b'", apply_id);
            try
            {
                DataTable dt = SqlHelper.DataTable(sql, CommandType.Text);
                return dt;
        ...
        public int SubmitCheckInForm(JW_Usedetail jwUsedetail, string unit_id, JW_Apply jwApply)
        {
            ...
            string checksql = string.Format(@"select * from JW_Usedetail where apply_id='{0}'", jwUsedetail.apply_id);
            try
            {
...
update JW_SendPolice set state=2 where type=2 and Object_id=@apply_id and user_id=@adduser_id and state=1;
...
--update JW_SendPolice set state=3 where type=2 and Object_id=@apply_id --and user_id=@user_id
                                            ");
            SqlParameter[] pars = new SqlParameter[]
            {
